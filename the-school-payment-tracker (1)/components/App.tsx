...
export const Logo = ({ className }: { className?: string }) => (
  <img 
    ...
    onError={(e) => {
      e.currentTarget.src = "https://placehold.co/400x400/ffffff/000000?text=The+School";
    }}
  />
);
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#F8FAFC] text-black">
      <div className="md:hidden bg-white border-b border-slate-200 p-4 flex justify-between items-center sticky top-0 z-50 print:hidden">
        <div className="flex items-center">
          <div className="w-10 h-10 rounded-lg overflow-hidden bg-white flex items-center justify-center shadow-sm border border-slate-100 shrink-0">
            <Logo className="w-full h-full p-1" />
          </div>
          <span className="ml-3 font-black text-black tracking-tight text-sm uppercase">The school</span>
        </div>
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-black hover:bg-slate-50 rounded-lg transition-colors">
          {isSidebarOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>

      <aside className={`
        fixed inset-y-0 left-0 z-40 w-full sm:w-72 bg-[#0F172A] transition-transform duration-300 transform 
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
        md:relative md:translate-x-0 flex flex-col shadow-2xl print:hidden
      `}>
        <div className="p-8 flex flex-col items-center border-b border-slate-800/50">
          <div className="relative w-full flex justify-center">
            <div className="w-40 h-40 sm:w-48 sm:h-48 aspect-square rounded-[2rem] sm:rounded-[2.5rem] bg-white shadow-2xl flex items-center justify-center overflow-hidden transition-all duration-500 hover:scale-[1.03]">
              <Logo className="w-full h-full p-4" />
            </div>
          </div>
        </div>

        <nav className="flex-1 px-6 space-y-2 mt-8 sm:mt-10 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => {
              ...
              className={`
                w-full flex items-center gap-4 px-5 py-4 rounded-xl sm:rounded-2xl transition-all duration-200
                ${activeTab === item.id 
                  ? 'bg-slate-800 text-white font-bold shadow-lg' 
                  : 'text-slate-400 hover:text-slate-100 hover:bg-slate-800/30'}
              `}
            >
              <item.icon size={18} className={activeTab === item.id ? 'text-sky-400' : 'text-slate-500'} />
              <span className="text-xs sm:text-sm font-semibold tracking-wide uppercase">{item.label}</span>
            </button>
          ))}
        </nav>
      </aside>
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-30 md:hidden print:hidden" onClick={() => setIsSidebarOpen(false)} />
      )}
